import { AcademicProjectResponseProfessorDTO, AcademicProjectResponseStudentDTO } from "@/features/projects/interfaces/projectInterfaces";
...
export const getProjectsByUserEmail = async (
    email: string
  ): Promise<(AcademicProjectResponseProfessorDTO | AcademicProjectResponseStudentDTO)[]> => {
    try {
      const response = await axios.get<AcademicProjectResponseProfessorDTO | AcademicProjectResponseStudentDTO[]>(
        "/projects/by-email",
        {
          params: { email }
        }
      ...
      // Verificar se a resposta é um array ou um único objeto
...
export const getAllProjectsForProfessor = async (): Promise<AcademicProjectResponseProfessorDTO[]> => {
  try {
    const response = await axios.get<AcademicProjectResponseProfessorDTO[]>("/projects/all-professor");
    return response.data;
  } catch (error) {
    if (error instanceof AxiosError && error.response) {
      throw new Error(error.response.data.message || "Erro ao listar projetos de professor");
    }
    throw new Error("Erro de conexão com o servidor");
  }
};
...
export const getAllProjectsForStudent = async (): Promise<AcademicProjectResponseStudentDTO[]> => {
  try {
    const response = await axios.get<AcademicProjectResponseStudentDTO[]>("/projects/all-student");
    return response.data;
  } catch (error) {
    if (error instanceof AxiosError && error.response) {
      throw new Error(error.response.data.message || "Erro ao listar projetos de aluno");
    }
    throw new Error("Erro de conexão com o servidor");
  }
};
...
/*
export const getAllProjects = async (): Promise<(AcademicProjectResponseProfessorDTO | AcademicProjectResponseStudentDTO)[]> => {
  try {
    const response = await axios.get<AcademicProjectResponseProfessorDTO | AcademicProjectResponseStudentDTO[]>("/projects/all");
    return response.data;
  } catch (error) {
    if (error instanceof AxiosError && error.response) {
      throw new Error(error.response.data.message || "Erro ao listar todos os projetos");
    }
    throw new Error("Erro de conexão com o servidor");
  }
};
